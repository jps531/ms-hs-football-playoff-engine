...
"""
Exact region outcome enumerator with per-game margin sweeps integrated into odds computation.
Adds --out-seeding to produce a human-readable seeding_odds.txt summary.
"""
...
def fetch_completed_pairs(conn, teams: List[str], season: int) -> List[CompletedGame]:
    ...
    for school, opp, result, pf, pa in rows:
        ...
        res = 1 if result == 'W' and school == a else -1 if result == 'W' and school == b else \
              -1 if result == 'L' and school == a else 1 if result == 'L' and school == b else 0
        d["res_a"] += res
    ...
    for (a,b), vals in tmp.items():
        res_a = 1 if vals["res_a"] > 0 else (-1 if vals["res_a"] < 0 else 0)
        out.append(CompletedGame(a,b,res_a, vals["pd_a"], vals["pa_a"], vals["pa_b"]))
    return out
...
def standings_from_mask(teams, completed, remaining, mask, pa_win, margins, base_margin_default=7):
    ...
    for cg in completed:
        if cg.res_a == 1: wlt[cg.a]["w"] += 1; wlt[cg.b]["l"] += 1
        elif cg.res_a == -1: wlt[cg.b]["w"] += 1; wlt[cg.a]["l"] += 1
        else: wlt[cg.a]["t"] += 1; wlt[cg.b]["t"] += 1
...
def build_h2h_maps(completed, remaining, mask, margins, base_margin_default=7):
    ...
    for cg in completed:
        ...
        else: pts[(cg.a,cg.b)]+=0.5; pts[(cg.b,cg.a)]+=0.5
    ...
    for i, rg in enumerate(remaining):
        ...
        if bit==1:
            pts[(rg.a,rg.b)]+=1.0; pd_cap[(rg.a,rg.b)]+=min(m,12); pd_cap[(rg.b,rg.a)]-=min(m,12)
            pd_uncap[(rg.a,rg.b)]+=m; pd_uncap[(rg.b,rg.a)]-=m
        else:
            pts[(rg.b,rg.a)]+=1.0; pd_cap[(rg.a,rg.b)]-=min(m,12); pd_cap[(rg.b,rg.a)]+=min(m,12)
            pd_uncap[(rg.a,rg.b)]-=m; pd_uncap[(rg.b,rg.a)]+=m
...
def tie_bucket_groups(teams, wlt):
    ...
    for s in order:
        ...
        w,l,t=wlt[s]["w"],wlt[s]["l"],wlt[s]["t"]
        gp=w+l+t; wp=(w+0.5*t)/gp if gp>0 else 0.0
        group=buckets[(round(wp,6),l)]; out.append(sorted(group)); seen.update(group)
    return out
...
def bucket_margin_games(bucket,remaining):
    b=set(bucket); return [rg for rg in remaining if rg.a in b and rg.b in b]
...
def enumerate_region(conn, clazz, region, season, out_csv=None, explain_json=None, out_seeding=None):
    ...
    if R==0:
        ...
        for mask in range(total):
            ...
            if K==0:
                ...
                game_keys=[(rg.a,rg.b) for rg in intra_games]
                weight=1.0/(12**K)
                for mtuple in product(range(1,13), repeat=K):
                    margins=dict(base_margins)
                    for key,mv in zip(game_keys,mtuple): margins[key]=mv
                    order=resolve_standings_for_mask(teams,completed,remaining,mask,margins=margins,base_margin_default=7,pa_win=pa_win)
                    slots=rank_to_slots(order)
                    for s,(a,b) in slots.items():
                        if 1>=a and 1<=b: first[s]+=weight
                        if 2>=a and 2<=b: second[s]+=weight
                        if 3>=a and 3<=b: third[s]+=weight
                        if 4>=a and 4<=b: fourth[s]+=weight

            if K>0:
                winners={}
                for i,rg in enumerate(remaining):
                    bit=(mask>>i)&1; winners[(rg.a,rg.b)]=(rg.a if bit==1 else rg.b)
                ...
                for bucket in buckets:
                    intra=bucket_margin_games(bucket,remaining)
                    if not intra or len(bucket)<=1: continue
                    baseline_places={t:baseline_order.index(t)+1 for t in bucket}
                    for rg in intra:
                        pair=(rg.a,rg.b); winner=winners[pair]; loser=rg.b if winner==rg.a else rg.a
                        threshold=None; snap=None
                        for m in range(1,13):
                            ...
                            test_places={t:test_order.index(t)+1 for t in bucket}
                            if any(baseline_places[t]!=test_places[t] for t in bucket):
                                threshold=m; snap={"before":baseline_places,"after":test_places}; break
                        if threshold is not None:
                            moves=[]; swings=[]
                            for t in bucket:
                                b=baseline_places[t]; a=snap["after"][t] # type: ignore
                                if b!=a:
                                    moves.append(f"{t} {ordinal(b)}→{ordinal(a)}")
                                    if (b<=4)!=(a<=4): swings.append(f"{t} {'into' if a<=4 else 'out of'} top‑4 ({ordinal(b)}→{ordinal(a)})")
                            sentence=(f"In this outcome, {winner} defeats {loser}. If the margin is ≥ {threshold}, the bucket {bucket} reorders: {', '.join(moves) if moves else 'ordering changes within tie group' }.")
                            if swings: sentence+=f" Playoff impact: {'; '.join(swings)}."
                            scenario_changes.append({"bucket":bucket,"game":f"{rg.a} vs {rg.b}","winner_by_mask":winner,"threshold_margin":threshold,"impact":snap,"summary":sentence})
                if scenario_changes: explanations.append({"mask":mask,"changes":scenario_changes})
        denom=float(1<<R)
    ...
    # Write seeding_odds.txt-style output
    if out_seeding:
        by_seed={1:[],2:[],3:[],4:[],'out':[]}
        for (school,o1,o2,o3,o4,op,fop,clinched,eliminated) in results:
            by_seed[1].append((school,o1)); by_seed[2].append((school,o2)); by_seed[3].append((school,o3)); by_seed[4].append((school,o4))
            by_seed['out'].append((school,1.0-op))
        for k in [1,2,3,4,'out']:
            by_seed[k].sort(key=lambda x:(-x[1],x[0]))
        def pct(x):
            val=x*100.0
            return f"{int(round(val))}%" if abs(val-round(val))<1e-9 else f"{val:.0f}%".rstrip('0').rstrip('.')
        header=f"Region {region}-{clazz}A"
        lines=[header,"","1 Seed:"]
        for seed in [1,2,3,4]:
            if seed!=1:
                lines.append(""); lines.append(f"{seed} Seed:")
            wrote=False
            for team,prob in by_seed[seed]:
                if prob>0: lines.append(f"{pct(prob)} {team}"); wrote=True
            if not wrote: lines.append("None")
        lines.append(""); lines.append("Eliminated:")
        eliminated_names=[team for team,prob in by_seed['out'] if abs(prob-1.0)<1e-12]
        if eliminated_names:
            for t in sorted(eliminated_names): lines.append(t)
        else:
            lines.append("None")
        with open(out_seeding,'w') as f:
            f.write("\n".join(lines)+"\n")
    ...
    if out_csv:
        with open(out_csv,'w',newline='') as f:
            w=csv.writer(f)
            w.writerow(["school","odds_1st","odds_2nd","odds_3rd","odds_4th","odds_playoffs","final_odds_playoffs","clinched","eliminated"])
            for row in sorted(results, key=lambda r: (-r[6], r[0])):
                w.writerow([row[0],f"{row[1]:.6f}",f"{row[2]:.6f}",f"{row[3]:.6f}",f"{row[4]:.6f}",f"{row[5]:.6f}",f"{row[6]:.6f}",row[7],row[8]])
        print(f"Wrote CSV: {out_csv}")

    if explain_json:
        with open(explain_json,'w') as f:
            json.dump(explanations,f,indent=2,sort_keys=True)
        print(f"Wrote explanation JSON: {explain_json}")
...
def main():
    ...
    with psycopg.connect(args.dsn) as conn:
        enumerate_region(conn, args.clazz, args.region, args.season, out_csv=args.out_csv, explain_json=args.explain_json, out_seeding=args.out_seeding)
