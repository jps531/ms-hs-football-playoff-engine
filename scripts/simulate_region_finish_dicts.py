...
def enumerate_region(conn, clazz, region, season, debug=False):
    """
    ...
    for team, seed_map in scenario_minterms.items():
        for seed_num, minterms in seed_map.items():
            ...
            minimized[seed_num][team] = minimized_mins

    with open('dicts.txt', "w") as f:
            f.write(json.dumps(minimized, indent=2))
    print(f"Wrote dicts text to dicts.txt")
...
def main():
    ...
    with psycopg.connect(args.dsn) as conn:
        enumerate_region(conn, args.clazz, args.region, args.season, debug=args.debug)
